...
1. **填写拐力**：在「队友拐力」和「自拐」中填写攻击力、暴击率、暴击伤害等，支持 `100+200` 这种累加格式。（记得加上遗器套装效果等）
2. **配置截图区域**：
   - `脱装备详情`：角色属性详情页区域（用于记录基础值，只脱遗器即可）  
      <img width="1358" height="504" alt="image" src="https://github.com/user-attachments/assets/f82285c8-39b6-4b99-b0bf-5d40dc36b186" />  
   - `遗器详情`：单件遗器详情区域  
      <img width="565" height="392" alt="image" src="https://github.com/user-attachments/assets/fbe99dce-1bdc-4c53-9093-f7b75f20ec25" />

   - 格式为 `左上x,左上y | 右下x,右下y`，例如 `100,200 | 660,590`
   - 点击「采样」后，移动到区域左上角按回车，再移动到右下角按回车
3. **预览截图**：点击「截图详情预览」或「截图遗器预览」确认实际截到的画面
4. **记录基础值**：角色遗器全脱后打开属性详情，按数字键 `1`
...
- 遗器总分下方如果有括号，那是遗器理论总分，即假设暴击不会溢出情况下计入额外暴击分
